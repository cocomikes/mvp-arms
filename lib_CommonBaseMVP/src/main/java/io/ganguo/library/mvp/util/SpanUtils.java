...
public final class SpanUtils {
    ...
    @IntDef({GRAVITY_BOTTOM, GRAVITY_CENTER, GRAVITY_TOP})
    @Retention(RetentionPolicy.SOURCE)
    ...
    private RoundBackgroundSpan roundBgSpan;
    private String url;
    ...
    public SpanUtils setRoundBackgroundSpan(@NonNull RoundBackgroundSpan span) {
        this.roundBgSpan = span;
        return this;
    }
    ...
    private void updateCharCharSequence() {
        ...
        if (roundBgSpan != null) {
            mBuilder.setSpan(roundBgSpan, start, end, flag);
        }
        if (url != null) {
    ...
    public static class RoundBackgroundSpan extends ReplacementSpan {
        ...
        protected @Gravity int gravity;

        public RoundBackgroundSpan(int bgColor, int textColor) {
            this(bgColor, textColor, 5f, GRAVITY_CENTER);
        }

        public RoundBackgroundSpan(int bgColor, int textColor, @Gravity int gravity) {
            this(bgColor, textColor, 5f, gravity);
        }

        public RoundBackgroundSpan(int bgColor, int textColor, float radius, @Gravity int gravity) {
        ...
        public void draw(@NonNull Canvas canvas, CharSequence text, @IntRange(from = 0) int start, @IntRange(from = 0) int end, float x, int top, int y, int bottom, @NonNull Paint paint) {
            if(this.gravity == GRAVITY_CENTER){
                float rectTop = top;                         // 矩形顶部y坐标
                float rectBottom = bottom;                   // 矩形底部y坐标

                // 文字行高
                float fontHeight = paint.getFontMetrics().bottom - paint.getFontMetrics().top;
                // 计算矩形中线位置
                float middleLine = (rectBottom + rectTop) / 2;

                RectF rect = new RectF(
                        x,
                        middleLine - fontHeight / 2,
                ...
                // 文字基线y坐标
                float baseLine = middleLine + (Math.abs(paint.getFontMetrics().ascent) - paint.getFontMetrics().descent) / 2;
                canvas.drawText(text, start, end, x, baseLine, paint);
            } else if(this.gravity == GRAVITY_BOTTOM){
                float fontHeight = paint.getFontMetrics().bottom - paint.getFontMetrics().top;
                float rectTop = bottom - fontHeight;     // 矩形顶部y坐标
                float rectBottom = bottom;              // 矩形底部y坐标
                // 计算矩形中线位置
                float middleLine = (rectTop + rectBottom) / 2;
                ...
                float baseLine = middleLine + (Math.abs(paint.getFontMetrics().ascent) - paint.getFontMetrics().descent) / 2;
                canvas.drawText(text, start, end, x, baseLine, paint);
            } else if(this.gravity == GRAVITY_TOP){
                float fontHeight = paint.getFontMetrics().bottom - paint.getFontMetrics().top;
                float rectTop = 0F;
                float rectBottom = fontHeight;
                // 计算矩形中线位置
                float middleLine = (rectTop + rectBottom) / 2;
    ...
    class CustomIconMarginSpan
        ...
        private Bitmap uri2Bitmap(final Uri uri) {
            try {
                return MediaStore.Images.Media.getBitmap(UIUtils.getContext().getContentResolver(), uri);
            } catch (IOException e) {
                e.printStackTrace();
        ...
        private Bitmap resource2Bitmap(final int resourceId) {
            Drawable drawable = ContextCompat.getDrawable(UIUtils.getContext(), resourceId);
    ...
    class CustomImageSpan extends CustomDynamicDrawableSpan {
        ...
        public Drawable getDrawable() {
            ...
            if (mDrawable != null) {
                ...
                Bitmap bitmap;
                try {
                    InputStream is = UIUtils.getContext().getContentResolver().openInputStream(mContentUri);
